...
export default class BleModule {
  constructor() {
    this.isConnecting = false; //Is the bluetooth connected?
    this.initUUID();
  ...
  //Get serviceUUID and characteristicUUID of Notify, Read, Write, WriteWithoutResponse
  getUUID(services) {
    this.readServiceUUID = [];
    this.readCharacteristicUUID = [];
    this.writeWithResponseServiceUUID = [];
  ...
  write(value, index) {
    let formatValue;
    if (value === "0D0A") {
      //End mark for sending receipt directly to printer
      formatValue = value;
    } else {
