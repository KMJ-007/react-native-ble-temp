...
export default class App extends Component {
      ...
      BluetoothManager.manager.startDeviceScan(null, null, (error, device) => {
        if (error) {
          ...
          if (error.errorCode == 102) {
            this.alert(
              "Please turn on your phone's bluetooth before searching"
            );
          }
          this.setState({ scaning: false });
        } else {
          console.log(device.id, device.name);
          this.deviceMap.set(device.id, device); //Use Map type to save the searched bluetooth devices, make sure the list does not show duplicate devices
  ...
  connect(item) {
    if (this.state.scaning) {
      //Scanning when connected, stop scanning first
      BluetoothManager.stopScan();
      this.setState({ scaning: false });
    }
